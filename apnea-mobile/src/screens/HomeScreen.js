...
const parseJsonField = (value, fallback = null) => {
  ...
  if (typeof value === 'string') {
    try {
      ...
      console.warn('Failed to parse Fitbit JSON field:', error);
      return fallback;
    }
  }
...
const normalizeFitbitRecord = (record) => ({
  ...record,
  sleep_data: parseJsonField(record.sleep_data, []),
  heart_rate_data: parseJsonField(record.heart_rate_data, []),
  hrv_data: parseJsonField(record.hrv_data, { hrv: [] }),
  activity_data: parseJsonField(record.activity_data, null),
});
...
const getMainSleep = (sleepData) => {
  ...
  return (
    sleepData.find((sleep) => sleep.isMainSleep) ||
    sleepData.find((sleep) => sleep.type === 'stages') ||
    sleepData[0]
  );
};
...
export default function HomeScreen({ navigation }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const normalizedData = (data || []).map(normalizeFitbitRecord);

      await processSleepData(normalizedData);
      await processHRVData(normalizedData);
      await processHeartRateData(normalizedData);

      const hasAnyValidFitbitData = normalizedData.some((record) => {
        const sleep = getMainSleep(record.sleep_data);
        const hasSleep = sleep && sleep.minutesAsleep > 0;
        const hasHR =
          Array.isArray(record.heart_rate_data) &&
          record.heart_rate_data.some((entry) => entry?.value?.restingHeartRate);
        const hasHRV =
          record.hrv_data?.hrv?.some((entry) => Array.isArray(entry?.minutes) && entry.minutes.length > 0);

        return hasSleep || hasHR || hasHRV;
      });

      if (!hasAnyValidFitbitData && currentWeekOffset > 0) {
  ...
  const processSleepData = async (data) => {
    ...
    for (const record of data) {
      ...
      if (sleep && sleep.minutesAsleep > 0 && weekMap[record.date]) {
        ...
        const deepHours = (summary.deep?.minutes || 0) / 60;
        const lightHours = (summary.light?.minutes || summary.asleep?.minutes || 0) / 60;
        const remHours = (summary.rem?.minutes || 0) / 60;
        const wakeHours = (sleep.minutesAwake || summary.wake?.minutes || summary.awake?.minutes || 0) / 60;
    ...
    const lastNightData = [...weeklyData].reverse().find((day) => day.hasData) || null;
  ...
  const processHRVData = async (data) => {
    ...
    const hrvValues = Object.values(weekMap).map((day) => ({
      date: day.date,
      dayName: day.dayName,
      value: null,
      hasData: false,
    ...
    for (const record of data) {
      const index = hrvValues.findIndex((item) => item.date === record.date);
      if (index === -1) continue;

      let dailyRMSSD = null;

      if (record.hrv_data?.hrv?.length > 0) {
        const hrvEntry = record.hrv_data.hrv[0];

        if (Array.isArray(hrvEntry?.minutes) && hrvEntry.minutes.length > 0) {
          let totalRMSSD = 0;
          let validMinutes = 0;

          for (const minute of hrvEntry.minutes) {
            const rmssd = minute?.value?.rmssd;

            if (typeof rmssd === 'number' && !Number.isNaN(rmssd)) {
              totalRMSSD += rmssd;
              validMinutes++;
            }
          }
          ...
          if (validMinutes > 0) {
            ...
            minHRV = Math.min(minHRV, dailyRMSSD);
            maxHRV = Math.max(maxHRV, dailyRMSSD);
          }
        }
      }
  ...
  const processHeartRateData = async (data) => {
    ...
    const heartRateValues = Object.values(weekMap).map((day) => ({
      date: day.date,
      dayName: day.dayName,
      value: null,
      hasData: false,
    ...
    for (const record of data) {
      const index = heartRateValues.findIndex((item) => item.date === record.date);
      if (index === -1) continue;

      let restingHR = null;

      if (Array.isArray(record.heart_rate_data)) {
        const heartRateEntry = record.heart_rate_data.find((entry) => entry?.value?.restingHeartRate);
        restingHR = heartRateEntry?.value?.restingHeartRate || null;
      }

      if (typeof restingHR === 'number' && !Number.isNaN(restingHR)) {
        totalRestingHR += restingHR;
        validHRDays++;
        minHR = Math.min(minHR, restingHR);
        maxHR = Math.max(maxHR, restingHR);
      }

      heartRateValues[index] = {
        ...heartRateValues[index],
        value: restingHR,
  ...
  const calculateSleepQuality = () => {
    ...
    const avgDeepHours = parseFloat(stats.avgDeepHours);
    const avgRemHours = parseFloat(stats.avgRemHours);

    if (avgHours >= 7) score += 30;
    else if (avgHours >= 6) score += 20;
    else if (avgHours >= 5) score += 10;

    const deepPct = avgHours > 0 ? avgDeepHours / avgHours : 0;
    if (deepPct >= 0.2) score += 30;
    else if (deepPct >= 0.15) score += 20;
    else if (deepPct >= 0.1) score += 10;

    const remPct = avgHours > 0 ? avgRemHours / avgHours : 0;
  ...
  const InteractiveLineChart = ({ title, data, unit, color, note, minValue, maxValue }) => {
      ...
      return (
        <View style={styles.noDataSmallContainer}>
          <Text style={styles.noDataSmallText}>
            No {title ? title.toLowerCase() : 'sleep'} data available for this week
          </Text>
        </View>
      );
    }
    ...
    if (values.length === 0) {
      return (
        <View style={styles.noDataSmallContainer}>
          <Text style={styles.noDataSmallText}>
            No valid {title ? title.toLowerCase() : 'sleep'} data available
          </Text>
        </View>
      );
    }
    ...
    return (
      <View style={styles.chartCard}>
        <View style={styles.chartHeader}>
          ...
          <Text style={[styles.chartValue, { color: color || '#6fc6a8' }]}>
            {Math.round(avgValue)} {unit || ''}
          </Text>
        </View>
        ...
        <View style={styles.svgContainer}>
          <Svg width={LINE_CHART_WIDTH} height={LINE_CHART_HEIGHT}>
            ...
            <SvgText
              x="30"
              y={getYPosition(actualMin + range * 0.75)}
              fontSize="10"
              fill="#95a5a6"
              textAnchor="end"
            >
            ...
            <SvgText
              x="30"
              y={getYPosition(actualMin + range * 0.5)}
              fontSize="10"
              fill="#95a5a6"
              textAnchor="end"
            >
            ...
            <SvgText
              x="30"
              y={getYPosition(actualMin + range * 0.25)}
              fontSize="10"
              fill="#95a5a6"
              textAnchor="end"
            >
            ...
            <Line
              x1="35"
              y1={getYPosition(actualMax)}
              x2={LINE_CHART_WIDTH - 10}
              y2={getYPosition(actualMax)}
              stroke="#2a2a2a"
              strokeWidth="0.5"
              strokeDasharray="4"
            />
            <Line
              x1="35"
              y1={getYPosition(actualMin + range * 0.5)}
              x2={LINE_CHART_WIDTH - 10}
              y2={getYPosition(actualMin + range * 0.5)}
              stroke="#2a2a2a"
              strokeWidth="0.5"
              strokeDasharray="4"
            />
            <Line
              x1="35"
              y1={getYPosition(actualMin)}
              x2={LINE_CHART_WIDTH - 10}
              y2={getYPosition(actualMin)}
              stroke="#2a2a2a"
              strokeWidth="0.5"
              strokeDasharray="4"
            />
            ...
            {linePath && linePoints.length > 0 ? (
              <Path
                d={`${linePath} L ${linePoints[linePoints.length - 1].x} ${
                  LINE_CHART_HEIGHT - 30
                } L ${linePoints[0].x} ${LINE_CHART_HEIGHT - 30} Z`}
            ...
            {points.map((point, index) => (
              <G key={index} {...getPointHandlers(point)} style={Platform.OS === 'web' ? { cursor: 'pointer' } : {}}>
                {point.hasData ? (
                  <>
                    <Circle cx={point.x} cy={point.y} r="8" fill={`${color || '#6fc6a8'}30`} />
                    <Circle cx={point.x} cy={point.y} r="5" fill={color || '#6fc6a8'} />
                  </>
                ) : (
            ...
            {tooltip ? (
              <G>
                ...
                />
                <SvgText
                  x={getTooltipPosition(tooltip).x}
                  y={getTooltipPosition(tooltip).y + 15}
                  fontSize="12"
                  fill={color || '#6fc6a8'}
                  fontWeight="bold"
                  textAnchor="middle"
                ...
                <SvgText
                  x={getTooltipPosition(tooltip).x}
                  y={getTooltipPosition(tooltip).y + 28}
                  fontSize="10"
                  fill="#95a5a6"
                  textAnchor="middle"
                >
      ...
      <View style={styles.chartContainer}>
        ...
        <View style={styles.chartBars}>
          {stats.weeklyData.map((day, index) => {
            ...
            const deepPct = totalHours > 0 ? Math.min((day.deepHours || 0) / totalHours, 1) : 0;
            const lightPct = totalHours > 0 ? Math.min((day.lightHours || 0) / totalHours, 1) : 0;
            const remPct = totalHours > 0 ? Math.min((day.remHours || 0) / totalHours, 1) : 0;
            const wakePct = totalHours > 0 ? Math.min((day.wakeHours || 0) / totalHours, 1) : 0;
            ...
            return (
              ...
              >
                ...
                {totalHours > 0 && !Number.isNaN(totalHours) ? (
  ...
  const ExpandedDayDetails = () => {
    ...
    const totalHours =
      (selectedDay.deepHours || 0) +
      (selectedDay.lightHours || 0) +
      (selectedDay.remHours || 0) +
      (selectedDay.wakeHours || 0);
  ...
  if (!isConnected) {
    return (
      <ScrollView
        ...
        contentContainerStyle={[styles.noDataContainer, { paddingTop: getHeaderPadding() }]}
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#6fc6a8']} />}
      >
        <Svg width="100" height="100" viewBox="0 0 24 24" fill="none">
          <Path
            ...
            fill="none"
          />
          <Path d="M12 8v4m0 4h.01" stroke="#6fc6a8" strokeWidth="2" strokeLinecap="round" />
        </Svg>
        ...
        <TouchableOpacity style={styles.connectButton} onPress={() => navigation.navigate('Analysis')}>
  ...
  return (
    <ScrollView
      style={styles.container}
      contentContainerStyle={[styles.contentContainer, { paddingTop: getHeaderPadding() }]}
      refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#6fc6a8']} />}
      showsVerticalScrollIndicator={false}
    >
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Sleep Insights</Text>
        <Text style={styles.headerSubtitle}>Welcome back, {user?.email?.split('@')[0] || 'User'}</Text>
      </View>

      {!hasValidData ? (
        <View style={styles.disclaimer}>
          <Text style={styles.disclaimerText}>Something went wrong loading your Fitbit data. Pull down to retry.</Text>
        </View>
      ) : null}
